...
ESP01S (RST) →  按鈕開關 (+) ( 或 傾斜開關 (訊號線S) )
ESP01S (GND) →  穩壓模組 (GND) + 傾斜開關 (GND) + 燈帶 (GND) + 充電模組 (-) + 鋰電池 (-) + 按鈕開關 (-) (或 傾斜開關 (GND))
...
```
![](/docs/0.jpg)
<br></br>上面是接按鈕的方式，可將其放在屁股位置，按壓屁股喚醒。下面用的是傾斜開關，搖晃喚醒。<br></br>
![](/docs/3.jpg)
...
- 先在麵包板上用按鍵（短按）驗證喚醒行為，再換成傾斜開關。

### 風險與注意
- 若傾斜開關是常低（水平放置時低電平），切勿直接接到 RST，因為這會在正常情況下持續將 RST 拉低，導致無法啟動。
